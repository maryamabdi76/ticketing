          ...
          <div class="col-12">
            <div class="card text-right">
                ...
                <form method="POST" action="/updateConcert" enctype="multipart/form-data">
                        @csrf
                        <input id="id" type="hidden" class="form-control " name="id" value="{{ $concert->id}}" >

                        <div class="form-group row">
                            <label for="name" class="col-md-4 col-form-label text-md-left">{{ __('نام') }}</label>

                            <div class="col-md-6">
                                <input id="name" type="text" class="form-control @error('name') is-invalid @enderror" name="name" value="{{ $concert->name}}" required autocomplete="name" autofocus>
                        ...
                        <div class="form-group row">
                            <label for="" class="col-md-4 col-form-label text-md-left">{{ __('عکس های کنسرت') }}</label>
                            <div class="col-md-6">
                                ...
                                <img class="img-fluid" src="{{asset('/').$photo->path}}" width="32%" alt=""/>
                                    ...
                                    @if($concert->Images()->get()->count()>1)
                                    <a href="/deleteConcertPhoto/{{$photo->id}}/{{$concert->id}}" class="btn btn-danger btn-sm" data-toggle="tooltip" title="حذف" onclick="return confirm('Are you sure you want to delete this item?');" >
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-6">
                                <textarea id="act" rows="8" class="form-control @error('act') is-invalid @enderror" name="act" required autocomplete="act" autofocus>{{ $concert->act}}</textarea>
                                @error('act')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-6">
                                <textarea id="team" rows="8" class="form-control @error('team') is-invalid @enderror" name="team" required autocomplete="team" autofocus>{{ $concert->team}}</textarea>
                                @error('team')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
                            ...
                            <div class="col-md-6">
                                 @foreach($genres as $v)
                                    ...
                                    <input class="form-check-input" name="genre[]" type="checkbox" id="{{$v->id}}" value="{{$v->id}}"
                                                       @foreach($concert->genres as $genre) @if($v->id==$genre->id) checked @endif @endforeach>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-6">
                                <textarea id="date" rows="8" class="form-control @error('date') is-invalid @enderror" name="date" required autocomplete="date" autofocus>{{ $concert->date}}</textarea>
                                @error('date')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
